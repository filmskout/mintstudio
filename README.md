...
2. **Mint** — one click packages the work into ERC-721 metadata whose attributes embed `text_proof_ref`, `image_proof_ref` and `tee_verified`, then mints on BOT Chain testnet (or a clearly-labeled **simulated** mint with a real, reproducible metadata SHA-256 when chain env is absent).
...
# export BOTCHAIN_RPC_URL=... BOTCHAIN_PRIVATE_KEY=... BOTCHAIN_NFT_CONTRACT=...
...
server/mint.js      # ethers v6 ERC-721 mintWithURI (loaded only when BOTCHAIN_* env set)
...
1. Real BOT Chain testnet contract deployment (skill scripts ready) → mainnet
